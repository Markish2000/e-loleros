const champion = {
  name: 'Vayne',
  image:
    // 'https://ddragon.leagueoflegends.com/cdn/img/champion/splash/Vayne_0.jpg',
    'https://ddragon.leagueoflegends.com/cdn/img/champion/splash/Amumu_0.jpg',
  role: 'LUCHADOR',
  difficulty: 'ALTA',
  history:
    'Shauna Vayne es una cazadora de monstruos demaciana letal y despiadada que ha dedicado su vida a buscar y destruir al demonio que asesinó a su familia. Provista de una ballesta que lleva en la muñeca y un corazón ansioso de venganza, Vayne solo es verdaderamente feliz cuando mata a practicantes de las artes oscuras o a sus creaciones, golpeando desde las sombras con una ráfaga de proyectiles de plata.',
  skills: [
    {
      tecla: 'PASIVA',
      name: 'CAZADORA NOCTÍVAGA',
      detail:
        'Vayne caza implacablemente a los malhechores y obtiene 45 de velocidad de movimiento cuando avanza hacia un campeón enemigo cercano.',
      video:
        'https://d28xe8vt774jo5.cloudfront.net/champion-abilities/0067/ability_0067_P1.webm',
    },
    {
      tecla: 'Q',
      name: 'PIRUETA',
      detail:
        'Vayne realiza una pirueta para preparar su próximo disparo, el cual infligirá daño adicional.',
      video:
        'https://d28xe8vt774jo5.cloudfront.net/champion-abilities/0067/ability_0067_Q1.webm',
    },
    {
      tecla: 'W',
      name: 'PROYECTILES DE PLATA',
      detail:
        'Vayne impregna sus proyectiles de un metal especial, tóxico para los seres maléficos. El tercer ataque o habilidad consecutivos contra el mismo objetivo inflige un porcentaje de la vida máxima del objetivo como daño verdadero adicional.',
      video:
        'https://d28xe8vt774jo5.cloudfront.net/champion-abilities/0067/ability_0067_W1.webm',
    },
    {
      tecla: 'E',
      name: 'CONDENA',
      detail:
        'Vayne saca una pesada ballesta y dispara un enorme proyectil contra su objetivo, infligiéndole daño y haciendo que retroceda. Si este impacta contra un obstáculo del terreno, queda empalado, aturdido y recibe daño adicional.',
      video:
        'https://d28xe8vt774jo5.cloudfront.net/champion-abilities/0067/ability_0067_E1.webm',
    },
    {
      tecla: 'R',
      name: 'HORA FINAL',
      detail:
        'Vayne se prepara para una confrontación épica y obtiene daño de ataque adicional, invisibilidad durante Pirueta, reducción de enfriamiento para Pirueta y más velocidad de movimiento adicional durante Cazadora noctívaga.',
      video:
        'https://d28xe8vt774jo5.cloudfront.net/champion-abilities/0067/ability_0067_R1.webm',
    },
  ],
  skins: [
    {
      name: 'Vayne ladrona de almas',
      image:
        'https://ddragon.leagueoflegends.com/cdn/img/champion/splash/Vayne_10.jpg',
    },
    {
      name: 'Vayne Centinela',
      image:
        'https://ddragon.leagueoflegends.com/cdn/img/champion/splash/Vayne_25.jpg',
    },
  ],
};

export default champion;
